...
# NOTE: ``torch`` / ``speechbrain`` are imported lazily inside the functions
# below. This keeps ``import app.main`` cheap (and testable) on machines that
# don't have the multi-hundred-MB ML stack installed.

logger = logging.getLogger("ach.embedding")
...
logger.debug("Path.symlink_to patched → fallback to copy on Windows.")
...
def _ensure_downloaded():
    ...
    logger.info("Downloading model to: %s", _SAVE_DIR)
...
def _load_model():
    ...
    if not (_SAVE_DIR / "hyperparams.yaml").exists():
        ...
        logger.info("Using cached model at: %s", _SAVE_DIR)
